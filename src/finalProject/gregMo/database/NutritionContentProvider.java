...
public class NutritionContentProvider extends ContentProvider {
	...
	public int delete(Uri uri, String whereClause, String[] whereArgs) {
		...
		case FOOD_ID:
			id = uri.getLastPathSegment();
			numberRowsAffected = nutritionDB.delete(FoodTable.TABLE_NAME,
					DateTable.COLUMN_ID + " = " + id 
					+ (!TextUtils.isEmpty(whereClause) ? 
							" AND (" + whereClause + ")" : "") , whereArgs);
			break;
...
//			break;
		case FOOD:
			...
			break;
		case FOOD_ID:
			rowsUpdated = nutritionDB.update(FoodTable.TABLE_NAME, values,
					FoodTable.COLUMN_ID + " = " + uri.getLastPathSegment()
					+ (!TextUtils.isEmpty(selection) ? " AND (" + selection + ")" : ""), 
					selectionArgs);
			break;
		default: 
